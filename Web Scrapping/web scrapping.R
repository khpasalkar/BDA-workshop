...
sb$winner <- gsub(pattern, "", sb$winner) #replaces all occurences with the alternate given, so here all space patterns are replaced by blank thus removing winner score from winner
sb$loser <- gsub(pattern, "", sb$loser) #same as above for loser column
head(sb)
write.csv(sb,"Superbowl",row.names = FALSE)
#gsub and sub are 2 ways to remove or subsetting of an element 
